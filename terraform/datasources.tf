...
data "oci_load_balancer_load_balancers" "lbs" {
  count = alltrue([local.install_node_problem_detector_kube_prometheus_stack, var.preferred_kubernetes_services == "public", local.deploy_from_operator]) ? 1 : 0
...
data "oci_load_balancer_load_balancers" "internal_lbs" {
  count = alltrue([local.install_node_problem_detector_kube_prometheus_stack, var.preferred_kubernetes_services == "internal", local.deploy_from_operator]) ? 1 : 0
