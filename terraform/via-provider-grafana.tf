...
resource "kubernetes_config_map_v1" "grafana_common_dashboards" {
  for_each   = alltrue([local.install_node_problem_detector_kube_prometheus_stack, local.deploy_from_local || local.deploy_from_orm]) ? local.grafana_common_dashboards : {}
...
resource "kubernetes_config_map_v1" "grafana_nvidia_dashboards" {
  for_each = (
    ...
    ) && alltrue([local.install_node_problem_detector_kube_prometheus_stack, local.deploy_from_local || local.deploy_from_orm]) ? local.grafana_nvidia_dashboards : {}
  )
...
resource "kubernetes_config_map_v1" "grafana_amd_dashboards" {
  for_each = (
    ...
    ) && alltrue([local.install_node_problem_detector_kube_prometheus_stack, local.deploy_from_local || local.deploy_from_orm]) ? local.grafana_amd_dashboards : {}
  )
...
resource "kubernetes_config_map_v1" "grafana_oci_dashboards" {
  for_each = alltrue([local.install_node_problem_detector_kube_prometheus_stack, local.setup_oci_metrics_exporter, local.deploy_from_local || local.deploy_from_orm]) ? local.grafana_oci_dashboards : {}
...
resource "kubernetes_config_map_v1" "grafana_alerts" {
  for_each   = alltrue([local.install_node_problem_detector_kube_prometheus_stack, local.deploy_from_local || local.deploy_from_orm]) ? local.grafana_alerts : {}
  depends_on = [helm_release.prometheus]
  metadata {
