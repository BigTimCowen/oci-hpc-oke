...
resource "helm_release" "prometheus" {
  count = alltrue([local.install_node_problem_detector_kube_prometheus_stack, local.deploy_from_local || local.deploy_from_orm]) ? 1 : 0
  depends_on = [
...
resource "time_sleep" "wait_for_lb_provisioning" {
  count = alltrue([local.install_node_problem_detector_kube_prometheus_stack, var.preferred_kubernetes_services != "public"]) ? 1 : 0
...
data "kubernetes_service_v1" "grafana_internal_ip" {
  count = alltrue([anytrue([local.deploy_from_orm, local.deploy_from_local]), local.install_node_problem_detector_kube_prometheus_stack, var.preferred_kubernetes_services != "public"]) ? 1 : 0
